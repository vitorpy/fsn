  ...
  if (local_4 != -1) {
    *param_3 = local_4;
    ...
    if (sVar1 < 0x401) {
      ...
      /*
     * BADDATA ANALYSIS (from binary @ 0x0043a110):
     * Function: FAMMonitorCollection
     *
     * Looking at this assembly snippet:
     * **1. What it does:**
     * Calls strlen() twice (on s0 and s1), adds the lengths + 2, then calls realloc() with those parameters. Returns 0.
     * **2. C pseudocode:**
     * ```c
     * int len0 = strlen(s0);
     * int len1 = strlen(s1);
     * void *ptr = *(void **)(sp_var + 4);  // from stack struct at offset 3288
     * void *buf = *(void **)(sp_var);
     * realloc(ptr, buf, s1, len0 + len1 + 2);
     * return 0;
     * ```
     * GP offsets decoded:
     * - gp-31776 (0x83e0) = strlen
     * - gp-30464 (0x8900) = realloc (5-arg SGI variant, or could be a custom buffer resize function)
     * The +2 accounts for null terminators when concatenating two strings into a reallocated buffer. This is likely the tail of FAMMonitorCollection preparing a combined path/filename string before returning success (0).
     */
halt_baddata();
}
