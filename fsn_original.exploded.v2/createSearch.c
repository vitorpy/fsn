...

{
  ...
  install_help_callback(uVar1,&searchHelp);
      ...
      /*
     * BADDATA ANALYSIS (from binary @ 0x00430ca8):
     * Function: createSearch
     *
     * Looking at this assembly snippet:
     * **What it does:**
     * 1. Clears a global byte flag (searchInProgress or similar at DAT+0x6f05)
     * 2. Calls XtVaCreateManagedWidget with parent widget, widget class string, args array, NULL terminator
     * 3. Stores the created widget in a global (searchTextField at DAT+0x6eec)
     * 4. Calls XtManageChild on the created widget
     * 5. Calls XtAddCallback on another widget with a callback function
     * **C pseudocode:**
     * ```c
     * searchInProgress = 0;  /* DAT+0x6f05 = false */
     * searchTextField = XtVaCreateManagedWidget(
     * parent,           /* a0 from sp+40 */
     * xmTextFieldWidgetClass,  /* a1 = gp[-32676] + 15808 = "XmTextField" or class ptr */
     * args,             /* a2 = sp+56 (arg list built earlier) */
     * NULL              /* a3 = 0, terminator */
     * );
     * XtManageChild(searchTextField);
     * XtAddCallback(someWidget,  /* from sp+48 */
     * XmNactivateCallback,  /* or similar resource string */
     * callbackFunc);
     * ```
     * The halt_baddata likely occurred because Ghidra couldn't track the XtVaCreateManagedWidget varargs properly or got confused by the GP-relative addressing for the widget class constant.
     */
halt_baddata();
}
