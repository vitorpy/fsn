...

{
  ...
  *(uint *)(curcontext + 0x30) = (uint)*(ushort *)(param_3 + 0x12);
  curcontext->scale_factor /* was: *(float *)(curcontext + 0x34) */ =
...
LAB_0040c5ac:
    ...
    *(undefined4 *)(*(int *)(curcontextwindows + 0x34) + 0x3c) = 0;
    ...
    gl_pop_state();
        ...
        /*
     * BADDATA ANALYSIS (from binary @ 0x0040c1a0):
     * Function: FUN_0040c1a0
     *
     * Looking at this assembly snippet:
     * **What it does:**
     * Loop epilogue that zeros three fields at offsets 188, 204, and 172 in array elements (stride 224 bytes). After loop, sets a byte flag at offset 3154 in curcontext to 1, then calls a function via GP-30860 before returning.
     * **C pseudocode:**
     * ```c
     * // Loop body (v0 is index*224, iterating until v0 == a0)
     * do {
     * dirnode = *curcontext_ptr;
     * children = dirnode->children;  // offset 52
     * children[index].field_bc = 0;  // offset 188
     * children[index].field_cc = 0;  // offset 204
     * children[index].field_ac = 0;  // offset 172
     * v0 += 224;  // sizeof(child_entry) = 0xe0
     * } while (v0 != limit);
     * // Post-loop
     * ctx = *curcontext_ptr;
     * ctx->byte_c52 = 1;           // offset 3154 (0xc52)
     * (*gp_minus_30860)();         // likely redraw or update function
     * return;
     * ```
     * The 224-byte stride and offsets 172/188/204 suggest clearing visibility/selection flags on child directory entries. The final flag set and function call likely triggers a redraw.
     */
halt_baddata();
  }
  iVar2 = getgdesc(0x10);
  if (iVar2 != 0) {
    ...
    if (iVar2 != 0) {
      ...
      curcontext->render_time_usec /* was: *(undefined4 *)(curcontext + 0xc4c) */ = 100000;
