...

{
  ...
  FUN_0042f284(uVar1);
      ...
      /*
     * BADDATA ANALYSIS (from binary @ 0x0040d3cc):
     * Function: FUN_0040d3cc
     *
     * Looking at the GP offsets and call pattern:
     * **1. What it does:**
     * This is the tail of a widget creation function. It creates an XmLabel widget with a compound string, then manages and realizes the parent widget, returning the parent.
     * **2. C pseudocode:**
     * ```c
     * // ... after previous call (likely XmStringCreateLocalized)
     * // s0 = parent widget, s1 = compound string from earlier
     * Widget label = XtVaCreateManagedWidget(
     * "label",           // a1 = gp[-32676] + (-23388) = string literal
     * xmLabelWidgetClass,// implicit or from earlier setup
     * s0,                // a0 = parent (s0)
     * XmNlabelString, s1,// a2 = compound string
     * NULL               // a3 = 0 (terminator)
     * );
     * XtManageChild(label);  // gp[-31864] = XtManageChild
     * XtRealizeWidget(s0);   // gp[-30756] = XtRealizeWidget
     * return s0;             // return parent widget
     * ```
     * GP offset mapping (from your GP_MAPPING.md):
     * - gp[-31492] (0x84fc) = XtVaCreateManagedWidget
     * - gp[-31864] (0x8388) = XtManageChild
     * - gp[-30756] (0x87dc) = XtRealizeWidget
     * - gp[-32676] (0x805c) = rodata base for string "label" or similar
     */
halt_baddata();
}
