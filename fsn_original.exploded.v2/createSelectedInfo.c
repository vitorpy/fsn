...

{
  ...
  *(undefined4 *)(curcontextwindows + 0x20) = uVar1;
  XtManageChild(*(undefined4 *)(curcontextwindows + 0x20));
      ...
      /*
     * BADDATA ANALYSIS (from binary @ 0x0041be34):
     * Function: createSelectedInfo
     *
     * Looking at this assembly:
     * 1. **What it does:** Fills an Xt argument array at s0 with 10 resource args (offsets 12-36), calls XtCreateManagedWidget with parent (from stack), resource name string, widget class, args array, and count=5. Stores result in structure at [s1]+0x20, then calls XtManageChild on the new widget.
     * 2. **C pseudocode:**
     * ```c
     * // s0 = args array (XtArg[10])
     * args[3] = t8;      // offset 12
     * args[4] = t9;      // offset 16
     * args[5] = v0;      // offset 20
     * args[6] = t0;      // offset 24
     * args[7] = v0;      // offset 28
     * args[8] = t1;      // offset 32
     * args[9] = *t2;     // offset 36 - dereferenced pointer
     * // gp-0x805c = string constant (resource name like "selectedInfo")
     * // gp-0x83d8 = XtCreateManagedWidget
     * Widget w = XtCreateManagedWidget(parent, "selectedInfo", widgetClass, args, 5);
     * (*s1)->field_0x20 = w;
     * // gp-0x8388 = XtManageChild
     * XtManageChild(w);
     * return parent;  // v0 loaded from stack offset 52
     * ```
     * Note: The arg count (5) suggests only 5 name/value pairs despite filling more slots - likely the first 5 pairs (offsets 0-36 = indices 0-9, but count=5 means pairs 0-4).
     */
halt_baddata();
}
