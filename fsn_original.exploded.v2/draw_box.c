...

{
  ...
  if (param_2 == 4) {
    ...
    if ((param_3 & 0x20) != 0) {
      ...
      /*
     * BADDATA ANALYSIS (from binary @ 0x00422278):
     * Function: draw_box
     *
     * Looking at this assembly snippet:
     * **What it does:**
     * Calls v3f() twice with vertex data from arrays indexed by loop counter (s2), using base pointer (s1) and stride from curcontext[0x6ff8]. Then calls endline() and returns.
     * **C pseudocode:**
     * ```c
     * // s1 = vertex array base, s2 = index, stride likely 12 (sizeof float[3])
     * v3f(s1 + (index * 12));           // First vertex
     * v3f(s1 + (curcontext[0x6ff8/4] * 12));  // Second vertex (paired vertex)
     * endline();
     * // function epilogue - restore s0,s1,s2,ra and return
     * ```
     * GP offsets resolved:
     * - gp-32664 (0x8068): curcontext pointer
     * - gp-32192 (0x8240): v3f function
     * - gp-32060 (0x82c4): endline function
     * This appears to be the tail of a line-drawing loop that draws edges between paired vertices in a box wireframe.
     */
halt_baddata();
}
